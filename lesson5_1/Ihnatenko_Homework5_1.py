...
# Треба відфільтрувати за критеріями: рік >= 2017, двигун >= 1.6, ціна <= 36000
filtered = []
...
for name, info in car_data.items():
    ...
    if year >= 2017 and engine >= 1.6 and price <= 36000:
        filtered.append((name, info))

# Треба відсортувати за ціною, а це індекс - 4
sorted_filtered = sorted(filtered, key=lambda x: x[1][4])

# Треба вивести перші 5 машин
for name, info in sorted_filtered[:5]:
    print(name, info)
